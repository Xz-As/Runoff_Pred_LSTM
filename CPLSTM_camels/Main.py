...
def load_forcing_discharge(basin: str) -> Tuple[pd.DataFrame, int]:
    ...
    #print(nums)
    Q_obs = df.Q
    area = areas[basin]# basin area(km ** 2)
    Q_obs = 28316846.592 * Q_obs * 86400 / (area * 10 ** 6)# calculating the 
...
class Model(nn.Module):
    ...
    def __init__(self, hidden_size: int, dropout_rate: float = 0.0, choices = False):
        """Initialize model
...
'数据设定'
#basins = ('01022500', '01031500', '02297310', '02361000', '02479300', '04063700', '08014500', '14222500')
basins = ('01022500', '01052500', '01031500', '01047000', '01054200', '01057000')
#basins = ('01031500', '01047000', '01054300', '01057000')
#basins = ('01054200', '01057000')
if __name__ == "__main__":
  for basin in basins:
    ...
    if len(ARGS) > 0:
        ...
        for opt, val in opts:
            if opt in ['-h', '--help']:
                print("no args for cp-lstm or '-n', '--nn' for lstm-only")
            elif opt in ['-n', '--nn']:
                choice = True
            elif opt == '-i':
                cal_p = val
    ...
    for ep in range(n_epochs[0], n_epochs[1], 20):
        if not choice:
            j = 0
            print(f'CP-lstm is running with {ep} total epoch\n')
        else:
            j = 1
        ...
        tr_loader = DataLoader(ds_train, batch_size=512, num_workers=1, shuffle=True)
        ...
        val_loader = DataLoader(ds_val, batch_size=2048, num_workers=1, shuffle=False)
        ...
        ds_test = yiluo_dataset('test', basin, seq_length=sequence_length, period="eval", dates=[int(start_date1 - start_date), int(end_date1 - start_date)],
                                means=means, stds=stds, choices=choice)
        test_loader = DataLoader(ds_test, batch_size=2048, num_workers=1, shuffle=False)
        print("dataset load succeeded")
